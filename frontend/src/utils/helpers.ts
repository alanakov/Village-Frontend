...
export function resolveProductCategories(products: Product[], categories: Category[]): Product[] {
  const categoryMap = new Map(categories.map((c) => [c.idCategory, c]))
  return products.map((p) => ({ ...p, category: p.category ?? categoryMap.get(p.categoryId) }))
}
